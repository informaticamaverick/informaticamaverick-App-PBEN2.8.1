...
interface PresupuestoDao {
    ...
    @Query("SELECT SUM(total) FROM presupuestos WHERE estado = 'Aprobado' AND prestadorId = :prestadorId")
    suspend fun getTotalGanancias(prestadorId: String): Double?

    @Query("SELECT SUM(total) FROM presupuestos WHERE estado = 'Aprobado' AND prestadorId = :prestadorId AND fecha >= :fromDate")
